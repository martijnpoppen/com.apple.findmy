...
export class GSASRPAuthenticator {
    ...
    derivePassword(protocol, password, salt, iterations) {
        return __awaiter(this, void 0, void 0, function* () {
            let passHash = new Uint8Array(yield util.hash(srp.h, stringToU8Array(password).buffer));
            if (protocol == 's2k_fo')
                passHash = stringToU8Array(util.toHex(passHash));
            const imported = yield subtle.importKey('raw', passHash, { name: 'PBKDF2' }, false, ['deriveBits']);
            const derived = yield subtle.deriveBits({
                name: 'PBKDF2',
                hash: { name: 'SHA-256' },
                iterations,
                salt,
